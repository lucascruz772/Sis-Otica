...
const getServicos = (): Servico[] => {
    const saved = localStorage.getItem("servicos");
    if (saved) {
        const arr = JSON.parse(saved);
        return arr.map((nome: string, i: number) => ({ id: i + 1, nome }));
    }
    return [];
};

const ServicosCadastro: React.FC = () => {
    const navigate = useNavigate();
    const [servicos, setServicos] = useState<Servico[]>(getServicos());
    const [search, setSearch] = useState("");
    const servicosFiltrados = servicos.filter((s) => s.nome.toLowerCase().includes(search.toLowerCase()));
    ...
    const handleEditSave = (id: number) => {
        const novos = servicos.map((s) => (s.id === id ? { ...s, nome: editNome } : s));
        setServicos(novos);
        localStorage.setItem("servicos", JSON.stringify(novos.map((s) => s.nome)));
        setEditId(null);
    ...
    const handleNovo = () => {
        ...
        const novo: Servico = { id: servicos.length ? servicos[servicos.length - 1].id + 1 : 1, nome: novoNome };
        const novos = [...servicos, novo];
        setServicos(novos);
        localStorage.setItem("servicos", JSON.stringify(novos.map((s) => s.nome)));
        setNovoNome("");
        setShowNovo(false);
            ...
            <div className="w-full min-w-0 px-2 sm:px-4 md:px-8 flex flex-col flex-1 items-start">
                <div className="w-full flex flex-col flex-1 md:pr-8 md:pl-2 xl:pr-16 xl:pl-6 items-start">
                    ...
                    <div className="w-full flex items-center mb-2">
                        ...
                        <h2 className="text-2xl font-bold text-left text-gray-900 dark:text-white pl-0">Serviços</h2>
                    </div>
                    ...
                    <div className="w-full max-w-2xl mb-4 ml-0 mr-auto flex flex-col md:flex-row md:items-center md:gap-4">
                        ...
                        <div className="flex gap-2 mt-2 md:mt-0">
                            <button className="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded" onClick={() => setShowNovo((v) => !v)}>Novo Serviço</button>
                        </div>
                    </div>
                    {showNovo && (
                    ...
                    <style>{`
            #servicos-scroll::-webkit-scrollbar { display: none !important; }
            #servicos-scroll { -ms-overflow-style: none !important; scrollbar-width: none !important; }
          `}</style>
                    <div
                        id="servicos-scroll"
                        ...
                        <table className="min-w-full w-full text-xs sm:text-sm text-left text-gray-900 dark:text-white">
                            ...
                            <tbody>
                                {servicosFiltrados.length === 0 && (
                                    <tr><td colSpan={3} className="px-2 sm:px-4 py-8 text-center text-gray-500 dark:text-gray-400 text-xs sm:text-sm">Nenhum serviço cadastrado.</td></tr>
                                )}
                                {servicosFiltrados.map((servico) => (
                                    <tr key={servico.id} className="hover:bg-gray-100 dark:hover:bg-gray-700 transition">
                                        <td className="px-2 sm:px-4 py-2">{servico.id}</td>
                                        ...
                                        <td className="px-2 sm:px-4 py-2 text-center">
                                            {editId === servico.id ? (
                                                <>
                                                    <button className="text-green-600 hover:text-green-800 mr-2" onClick={() => handleEditSave(servico.id)} title="Salvar">✔</button>
                                                    <button className="text-red-600 hover:text-red-800" onClick={handleEditCancel} title="Cancelar">✖</button>
                                                </>
                                            ) : (
                                                <>
                                                    <button className="text-blue-600 hover:text-blue-800 mr-2" onClick={() => handleEdit(servico)} title="Editar">✎</button>
                                                    <button className="text-red-600 hover:text-red-800" onClick={() => setDeleteId(servico.id)} title="Excluir">🗑</button>
                                                </>
                                            )}
                                        </td>
                                    </tr>
                    ...
                    {deleteId !== null && (
                        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-40 z-50">
                            <div className="bg-white dark:bg-gray-800 rounded shadow-lg p-6 w-full max-w-sm">
                                ...
                                <p className="mb-4 text-gray-700 dark:text-gray-300">Deseja realmente excluir este serviço?</p>
