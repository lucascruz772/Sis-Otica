...
const getLaboratorios = (): Laboratorio[] => {
    const saved = localStorage.getItem("laboratorios");
    if (saved) {
        const arr = JSON.parse(saved);
        return arr.map((nome: string, i: number) => ({ id: i + 1, nome }));
    }
    // MOCK PADRÃO SE NÃO HOUVER NADA NO LOCALSTORAGE
    const mock = ["Lab Vision", "Óptica Brasil", "Lentes Premium"];
    return mock.map((nome, i) => ({ id: i + 1, nome }));
};

const LaboratorioCadastro: React.FC = () => {
    const navigate = useNavigate();
    const [laboratorios, setLaboratorios] = useState<Laboratorio[]>(getLaboratorios());
    const [search, setSearch] = useState("");
    const laboratoriosFiltrados = laboratorios.filter((l) => l.nome.toLowerCase().includes(search.toLowerCase()));
    ...
    const handleEdit = (lab: Laboratorio) => {
        setEditId(lab.id);
        setEditNome(lab.nome);
    };
    const handleEditSave = (id: number) => {
        const novos = laboratorios.map((l) => (l.id === id ? { ...l, nome: editNome } : l));
        setLaboratorios(novos);
        localStorage.setItem("laboratorios", JSON.stringify(novos.map((l) => l.nome)));
        setEditId(null);
    ...
    const handleNovo = () => {
        ...
        const novo: Laboratorio = { id: laboratorios.length ? laboratorios[laboratorios.length - 1].id + 1 : 1, nome: novoNome };
        const novos = [...laboratorios, novo];
        setLaboratorios(novos);
        localStorage.setItem("laboratorios", JSON.stringify(novos.map((l) => l.nome)));
        setNovoNome("");
        setShowNovo(false);
    ...
    const handleDelete = (id: number) => {
        const novos = laboratorios.filter((l) => l.id !== id);
        setLaboratorios(novos);
        localStorage.setItem("laboratorios", JSON.stringify(novos.map((l) => l.nome)));
        setDeleteId(null);
                ...
                <div className="w-full flex flex-col flex-1 md:pr-8 md:pl-2 xl:pr-16 xl:pl-6 items-start">
                    ...
                    <div className="w-full max-w-2xl mb-4 ml-0 mr-auto flex flex-col md:flex-row md:items-center md:gap-4">
                        ...
                        <div className="flex gap-2 mt-2 md:mt-0">
                            <button className="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded" onClick={() => setShowNovo((v) => !v)}>Novo Laboratório</button>
                        </div>
                    </div>
                    {showNovo && (
                        ...
                        <table className="min-w-full w-full text-xs sm:text-sm text-left text-gray-900 dark:text-white">
                            ...
                            <tbody>
                                {laboratoriosFiltrados.length === 0 && (
                                    <tr><td colSpan={3} className="px-2 sm:px-4 py-8 text-center text-gray-500 dark:text-gray-400 text-xs sm:text-sm">Nenhum laboratório cadastrado.</td></tr>
                                )}
                                {laboratoriosFiltrados.map((lab) => (
                                    <tr key={lab.id} className="hover:bg-gray-100 dark:hover:bg-gray-700 transition">
                                        ...
                                        <td className="px-2 sm:px-4 py-2 text-center">
                                            {editId === lab.id ? (
                                                <>
                                                    <button className="text-green-600 hover:text-green-800 mr-2" onClick={() => handleEditSave(lab.id)} title="Salvar">✔</button>
                                                    <button className="text-red-600 hover:text-red-800" onClick={handleEditCancel} title="Cancelar">✖</button>
                                                </>
                                            ) : (
                                                <>
                                                    <button className="text-blue-600 hover:text-blue-800 mr-2" onClick={() => handleEdit(lab)} title="Editar">✎</button>
                                                    <button className="text-red-600 hover:text-red-800" onClick={() => setDeleteId(lab.id)} title="Excluir">🗑</button>
                                                </>
                                            )}
                                        </td>
                                    </tr>
